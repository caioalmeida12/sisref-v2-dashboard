 ...
 */
export const pegarStatusDaRefeicao = (
  props: TRefeicaoECardapio,
): StatusDaRefeicao => {
  ...
  const dataHoraDoComecoDaRefeicao = DatasHelper.compilarDataHora(
    props.menu.date,
    props.meal.timeStart,
  );
  const diferencaEmHorasAteOComeco = DatasHelper.getDiferencaEmHoras(
    dataHoraDoComecoDaRefeicao,
  );

  const dataHoraDoFimDaRefeicao = DatasHelper.compilarDataHora(
    props.menu.date,
    props.meal.timeEnd,
  );
  const diferencaEmHorasAteOFim = DatasHelper.getDiferencaEmHoras(
    dataHoraDoFimDaRefeicao,
  );

  if (diferencaEmHorasAteOComeco < 0 || diferencaEmHorasAteOFim < 0)
    return "encerrado";
  if (diferencaEmHorasAteOComeco > props.meal.qtdTimeReservationStart)
    return "indisponivel";
  if (diferencaEmHorasAteOComeco < props.meal.qtdTimeReservationEnd)
    return "indisponivel";
  return "disponivel";
};
