...
import {
  TAgendamento,
  TAgendamentoSchema,
} from "@/app/interfaces/TAgendamento";
...
export async function criarRefeição(formData: FormData) {
  ...
  if (camposInvalidos.length) {
    return {
      sucesso: false,
      mensagem: `Os campos ${camposInvalidos.join(", ")} são obrigatórios.`,
    };
  }
...
export async function criarCardapio(formData: FormData) {
  ...
  if (
    ...
    typeof resposta.resposta[0] === "object" &&
    resposta.resposta[0] !== null
  ) {
    const primeiro_erro = resposta.resposta[0] as { [key: string]: unknown };

    if (
      "description" in primeiro_erro &&
      Array.isArray(primeiro_erro.description)
    ) {
 ...
 * @deprecated
 */
export async function deprecated_criarRelatorioDeDesperdicio(
  formData: FormData,
) {
  const resposta = await FetchHelper.post<unknown>({
 ...
 */
export async function buscarRelatorioDeRefeicoes({
  data_inicial,
  data_final,
}: {
  data_inicial?: string;
  ...
  | { sucesso: false; mensagem: string }
  | { sucesso: true; resposta: TRelatorioDeRefeicoes[] }
> {
