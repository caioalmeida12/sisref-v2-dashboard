...
import { IIconeStatusProps } from "@/app/interfaces/IIcones";
...
export const StatusDaRefeicao = ({
  texto,
  cor,
  icone,
  textoTooltip,
}: StatusDaRefeicaoProps) => {
  return (
    <CustomTooltipWrapper
      elementoTrigger={
        <div className="flex items-center gap-x-2 leading-normal">
          ...
          <Icone.Status
            fill={classNamePorCor[cor].fill || "fill-cinza-600"}
            variante={icone}
          />
        </div>
      }
      elementoContent={<p>{textoTooltip}</p>}
    />
  );
};
