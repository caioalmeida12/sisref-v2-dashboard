...
export const Modal = ({ children, titulo }: { children: React.ReactNode, titulo: string }) => {
  const { modalRef, handleClose } = useModal();

  const handleKeyDown = (event: React.KeyboardEvent) => event.key === "Enter" && handleClose();

  return (
    <dialog ref={modalRef} className="fixed inset-8 z-10 max-h-[calc(100svh-8rem)] max-w-[768px] bottom-8 my-8 px-6 py-4 rounded backdrop:bg-preto-400/15 backdrop:backdrop-blur-sm overflow-y-auto">
